...
设 \(f(x)\) 定义在 \([a,b]\) 上。若存在 \(x^*\in[a,b]\)，使 \(x^*\) 是 \(f\) 在 \([a,b]\) 上的最小点，并且对任意 \(a\le x_1<x_2\le b\)，满足：
...
初始点 \(x_1\) 充分接近 \(x^*\)，则 Newton 法产生的序列 \(\{x_k\}\) 二阶收敛于 \(x^*\)。
...
2. 计算 \(v=f'(a)\)，若 \(|v|<\varepsilon\)，停止，取 \(x^*=a\)；
3. 根据 \(v\) 的符号确定搜索方向：若 \(v<0\)，向右搜索；若 \(v>0\)，向左搜索；
4. 取 \(b=a+h\)，计算 \(u=f'(b)\)；
5. 若 \(|u|<\varepsilon\)，停止，取 \(x^*=b\)；
